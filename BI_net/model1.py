...
class BiFormerBlock(nn.Module):
    ...
    def forward(self, x, c_a):
        ...
        Return:
            NCHWD tensor
            c_a: channel attention from convnet

        """
        # cross-channel attention

        # attention & mlp
        x = x + self.drop_path(self.attn(self.norm1(x)))  # (N, C, H, W) (x + ca)    、、+ self.PAM(c_a)
...
class FusionNet(nn.Module):
    ...
    def __init__(self, n_win=None, topk=None,
        ...
        self.stem = nn.Sequential(nn.Conv3d(dim[0], dim[1], kernel_size=4, stride=2, padding=1),
                                  self.norm_layer(dim[1]),
                                  nn.ReLU(inplace=True),
                                  nn.Conv3d(dim[1], dim[2], kernel_size=4, stride=2, padding=1),
                                  nn.BatchNorm3d(dim[2]),
                                  nn.ReLU(inplace=True),

                                  # nn.Conv3d(dim[1], dim[2], kernel_size=1, stride=1, padding=0),
                                  # nn.BatchNorm3d(dim[2]),
                                  # nn.MaxPool3d(2),nn.ReLU(inplace=True),
                                  )
        ...
        for i in range(2):
            ...
            downsample_layer = nn.Sequential(
                self.norm_layer(dim[i + 2]),
                nn.Conv3d(dim[i + 2], dim[i + 3], kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            )
        ...
        for i in range(5):
            # patch merging: norm-conv
            if i == 0:
                pconvblock = nn.Sequential(
                    MLPBlock(dim[i + 2], 4, mlp_ratio=2, drop_path=self.drop_path, layer_scale_init_value=0,
                             act_layer=nn.ReLU,
                             norm_layer=nn.BatchNorm3d, pconv_fw_type='split_cat'),
                )
            elif i == 1:
                pconvblock = MLPBlock(dim[i + 2], 4, mlp_ratio=2, drop_path=self.drop_path, layer_scale_init_value=0,
                                      act_layer=nn.ReLU,
                                      norm_layer=nn.BatchNorm3d, pconv_fw_type='split_cat', double_input=True)
            else:
                pconvblock = MLPBlock(dim[4], 4, mlp_ratio=2, drop_path=self.drop_path, layer_scale_init_value=0,
                                      act_layer=nn.ReLU,
                                      norm_layer=nn.BatchNorm3d, pconv_fw_type='split_cat', double_input=True)
