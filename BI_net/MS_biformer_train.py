...
def warm_up_cosine_lr_scheduler(optimizer, epochs=100, warm_up_epochs=5, eta_min=5e-6):
    """
    Description:
...
class Trainer:
    ...
    def fit(self, epochs, train_loader, valid_loader, modility, save_path, floder_path, patience, fold, BS,
        ...
        for n_epoch in range(start_epoch + 1, epochs + 1):
            ...
            a = self.optimizer.param_groups[0]['lr']  # 使用coine_warmup策略时打印每个epoch的学习率
            ...
            if bool_for_different_traintype:
                ...
                self.save_model(n_epoch, modility, save_path, floder_path, current_val_loss=valid_loss, auc=valid_auc,
                                ...
                                fold_datasets_path=fold_datasets_path)
                # 打印信息
                self.info_message(
        ...
        make_train_pic(modility, fold, self.acc_best, train_acc_list, train_losses_list, val_acc_list,
                       folder_path=floder_path, val_losses_list=val_losses_list, save_path=save_path)
    ...
    def save_model(self, n_epoch, modility, save_path, floder_path, current_val_loss, auc, fold, all_epochs, patience,
                   current_val_acc, BS, val_acc_list, val_losses_list, fold_datasets_path):
...
def train_mri_type(mri_type, data_path, model_save_path, model_floder, fold_path_name_list, RESUME=None,
    ...
    for fold, fold_dir in enumerate(fold_path):
        ...
        # scheduler = warm_up_cosine_lr_scheduler(optimizer, epochs=epoch, warm_up_epochs=5)
...
if __name__ == "__main__":
    ...
    methon_model_name = 'MS_biformerV6'
    print(methon_model_name)
    data_name = ('date5_22')
    fold_path_name_list = ['f5', 'f3', 'f4', 'f1', 'f2']  # 'f1', 'f3', 'f4', 'f5',  'f2', 'f3', 'f4', 'f1', 'f5'
    ablation_section_name: str = ('lr5e-5_bf222_conv1222_comlp2222_bfmlp222_RETEST'
                                  '.2__headonoFc_double_spattGG')
    print(ablation_section_name)
    ...
    model_name = f'l{ablation_section_name}'
    model_root_path = f'{methon_model_name}_cloud/{target_name}_{len(fold_path_name_list)}fold/{data_name}'
    # ---------------对上次训练异常中断的结果继续训练------------------
