...
class InvertedResidualsBlock(nn.Module):
    def __init__(self, in_channels, expansion, stride):
        ...
        self.basic_block = nn.Sequential(
            nn.Conv3d(in_channels, channels, kernel_size=3, stride=stride, padding=1, groups=in_channels, bias=False),
...
class nchwBiFormerSTL(nn.Module):
    ...
    def __init__(self, in_chans=1, num_classes=1,
        ...
        for i in range(3):
            invertConv = Conv_branch(in_channels=embed_dim[i])
        ...
        for i in range(3):
            stage = BasicLayer(dim=embed_dim[i + 1],
                               depth=depth[i],
...
def MS_biformer():
    model = nchwBiFormerSTL(in_chans=1, num_classes=1,
                            depth=[1, 1, 3],  # 1, 1, 3
                            embed_dim=[32, 64, 128, 576],  # 64, 128, 576
                            mlp_ratios=[1, 1, 1],
                            head_dim=16,  # 16
                            drop_path_rate=0.0,
