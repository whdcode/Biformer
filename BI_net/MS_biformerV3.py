...
class Conv_branch(nn.Module):
    def __init__(self, in_channels, invert_exp=2, invert_s=1):
        super(Conv_branch, self).__init__()
        self.invert_convblock = InvertedResidualsBlock(in_channels, expansion=invert_exp, stride=invert_s)
    ...
    def forward(self, x_bf, x_co):
        ...
        x_co = self.invert_convblock(x_co) * score
        return x_co
...
class nchwBRA(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_scale=None, topk=4, side_dwconv=3, auto_pad=False,
        ...
        # 判断是否为每个stage的第一个block
        if scale is None:
            self.ms = nn.Identity()
        else:
            self.ms = Mlp(dim, scale=scale)
...
class BiFormerBlock(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., num_heads=8, n_win=7,
        ...
        # self.mlp = Mlp(dim, int(mlp_ratio * dim), drop_rate=drop_path)
...
class MSBiFormer(nn.Module):
    ...
    def __init__(self, in_chans=1, num_classes=1,
        ...
        # first MS and one MS in first stage
        # self.f_MS = Mlp(embed_dim[0], scale=(1, 3, 5, 7))
        # biformer ds
...
def MS_biformer():
    model = MSBiFormer(in_chans=1, num_classes=1,
                       ...
                       side_dwconv=5,  # 5
                       data_flows='conv_bf',
                       scale=[(1, 3, 5, 7), (1, 3, 5), (1, 3)])  # conv_bf or only_conv
