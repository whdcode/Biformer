...
class MSBiFormer(nn.Module):
    ...
    def __init__(self, in_chans=1, num_classes=1,
        ...
        for i in range(self.stage_nums - 1):
            ...
            self.downsample_layers.append(downsample_layer_bf)
        ...
        for i in range(self.stage_nums):
            invertConv = Other_Conv_BasicLayer(dim=embed_dim[i + 1], depth=co_depth[i + 1],
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        for i in range(self.stage_nums - 1):
            ...
            bf_out_per_stage.append(x)
            x = self.downsample_layers[i](x)
        x = self.bf_stages[-1](x)
        bf_out_per_stage.append(x)
        x = self.last_bf_norm(x)

        #  Conv_branch
        for i in range(self.stage_nums - 1):
            x_co = self.invertConvbranch[i](bf_out_per_stage[i], x_co)
            x_co = self.downsample_layers_co[i](x_co)
        x_co = self.invertConvbranch[-1](bf_out_per_stage[-1], x_co)
        return x, x_co

    def forward(self, x: torch.Tensor):
        if self.data_flows == 'conv_bf':
            x_bf, x_co = self.forward_features(x)
            x = torch.cat([x_bf.mean([2, 3, 4]), x_co.mean([2, 3, 4])], dim=1)
            # x = x_bf.mean([2, 3, 4]) + x_cov.mean([2, 3, 4])
            x = self.head(x)
        elif self.data_flows == 'only_conv':
            _, x_co = self.forward_features(x)
            x = x_co.mean([2, 3, 4])
            x = self.head(x)
        return x
