...
class SpatialAttention_Up(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.sigmoid(self.conv(out)) * x
...
class MSBiFormer(nn.Module):
    ...
    def __init__(self, in_chans=1, num_classes=1,
        ...
        self.fu_co = nn.ModuleList()
        for i in range(self.stage_nums - 1):
            self.fu_co.append(nn.Conv3d(embed_dim[i + 1] + embed_dim[i + 2], embed_dim[i + 2], 1))
            self.fu_co.append(nn.Conv3d(2 * embed_dim[i+2], embed_dim[i+2], 1))


        self.spa_att = nn.ModuleList()
        for i in range(self.stage_nums):
            sa = SpatialAttention_Up()
            self.spa_att.append(sa)

        self.ca_att = nn.ModuleList()
        for i in range(self.stage_nums - 1):
            ca = ECA_block(embed_dim[i + 1])
            self.ca_att.append(ca)

        ################################classification##########################################
        self.last_bf_norm = pre_head_norm_layer(embed_dim[-1] * 2)
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        # first_stage
        x_co = self.invertConvbranch[0](x)
        x_co_info = self.spa_att[0](x_co)
        x_bf = self.bf_stages[0](x_co_info)
        x_bf_info = self.ca_att[0](x_bf)
        # second_stage
        x_co = self.downsample_layers_co[0](x_co)
        x_bf = self.downsample_layers_bf[0](x_bf)
        x_co = self.fu_co[0](torch.cat([x_co, x_bf_info], dim=1))

        x_co = self.invertConvbranch[1](x_co)
        x_co_info = self.spa_att[1](x_co)
        x_co_info = self.fu_co[1](torch.cat([x_bf, x_co_info], dim=1))
        x_bf = self.bf_stages[1](x_co_info)
        x_bf_info = self.ca_att[1](x_bf)
        # third_stage
        x_co = self.downsample_layers_co[1](x_co)
        x_bf = self.downsample_layers_bf[1](x_bf)
        x_co = self.fu_co[2](torch.cat([x_co, x_bf_info], dim=1))

        x_co = self.invertConvbranch[2](x_co)
        x_co_info = self.spa_att[2](x_co)
        x_co_info = self.fu_co[3](torch.cat([x_bf, x_co_info], dim=1))
        x_bf = self.bf_stages[2](x_co_info)
        out = torch.cat([x_co, x_bf], dim=1)
