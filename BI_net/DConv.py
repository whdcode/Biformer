...
class DeformConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        q_sss = Variable(p.data, requires_grad=False).floor()  # point with all smaller coords 左下前000
        #         q_sss = p.data.floor() - same? / torch.Tensor(p.data).floor()
        q_lll = q_sss + 1  # all larger coords 右上后111
        ...
        q_ssl = torch.cat([q_sss[..., :N], q_sss[..., N:2 * N], q_lll[..., 2 * N:]], -1)    # 左前上001
        q_sls = torch.cat([q_sss[..., :N], q_lll[..., N:2 * N], q_sss[..., 2 * N:]], -1)    # 左后下010
        q_sll = torch.cat([q_sss[..., :N], q_lll[..., N:2 * N], q_lll[..., 2 * N:]], -1)    # 左后上011
        q_lss = torch.cat([q_lll[..., :N], q_sss[..., N:2 * N], q_sss[..., 2 * N:]], -1)    # 右前下100
        q_lsl = torch.cat([q_lll[..., :N], q_sss[..., N:2 * N], q_lll[..., 2 * N:]], -1)    # 右前上101
        q_lls = torch.cat([q_lll[..., :N], q_lll[..., N:2 * N], q_sss[..., 2 * N:]], -1)    # 右后下110
...
if __name__ == "__main__":
    input = torch.rand((8, 64, 56,56, 56)).to("cuda:1")
    net = DeformConv3d(64, 64)
    net.to("cuda:1")
